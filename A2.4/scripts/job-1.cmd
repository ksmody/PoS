...
do
	...
	do
		for partition in "classic"
		do
			for run in "run1" "run2" "run3"
			do
export SCOREP_EXPERIMENT_DIRECTORY="SCOREP/01/${input}/${ptype}/${partition}/01/${run}"
mpiexec -n 1 ./gccg $input $partition $ptype
echo "\n $input $partition $ptype 1"
			done
		done
	done
done