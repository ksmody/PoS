...
void createList(int nsend, int nextc, int **epart_send, int **epart_recv, int **sendl,
		int nprocs, int ne, int **lcc, int *nghb_cnt, int **nghb_to_rank,
	...
	*nghb_to_rank = (int*) calloc(sizeof(int), *nghb_cnt);
	...
	int nghost = 0;
	for (i = 0; i < *nghb_cnt; ++i) {
		recv_pos[i + 1] = recv_pos[i] + (*recv_cnt)[i];
	}
	for (i = 0; i < ne; ++i) {
		for (j = 0; j < 6; ++j) {
			ngh = lcc[i][j];
			if (ngh >= ne) {
				...
				if (n < ne) {
					if (n != -1) nghost++;
					n = ne + recv_pos[(*nghb_cnt + 1 + n) % (*nghb_cnt + 1)]++;
					(*epart_recv)[ngh - ne] = n;
				}
				lcc[i][j] = n;
			}
...
int initialization(char* file_in, char* part_type, char* read_type, int nprocs,
	...
	createList(nsend, *nextcf - *nintcf, &epart_send, &epart_recv, &sendl, nprocs, *nintcf + 1,
			   *lcc, nghb_cnt, nghb_to_rank, send_cnt, recv_cnt, send_lst, recv_lst);
