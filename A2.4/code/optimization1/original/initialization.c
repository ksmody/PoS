...
int initialization(char* file_in, char* part_type, char* read_type, int nprocs,
	...
	if (myrank == 0 || strcmp(read_type, "allread") == 0) {
		 ...
		 * Definition of mappings between global and local indeces for internal cells */
		for (i = 0; i <= nintcf_m; ++i) {
			s = displs[epart[i]];
			k = displs2[epart[i]]++;
			bp_m[k] = bp_m0[i];
	...
	*lcc = (int**) malloc((*nintcf) * sizeof(int*));
	...
	/* Distribution of the arrays, needed for defining the send- and receive-lists */
	if (strcmp(read_type, "oneread") == 0) {
		if (myrank == 0) {
			for (i = 0; i < nprocs; ++i) {
	...
	/* Definition of send- and receive-lists */
	...
	*nghb_cnt = 0;
	...
	recv_pos = (int*) calloc(sizeof(int), *nghb_cnt + 1);
