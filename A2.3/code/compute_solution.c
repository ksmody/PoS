...
//	int j;
	...
	/* Send - Receive Buffers for Communication of Ghost Cells*/
...
//	}
	...
	/*Derived Data Types for Communication of Ghost Cells*/
	MPI_Datatype sendtype[nghb_cnt];
	MPI_Datatype recvtype[nghb_cnt];
	for (i = 0; i < nghb_cnt; ++i) {
...
//	double t1 = MPI_Wtime();
	MPI_Request request[2 * nghb_cnt];
	...
	while (iter < max_iters) {
		...
		/*Loading Direc1 values into BUFFERS*/
//		for (i = 0; i < nghb_cnt; ++i) {
//			for (j = 0; j < send_cnt[i]; ++j) {
//				sendbuf[i][j] = direc1[send_lst[i][j]];
//			}
//		}
		/*Communication of Ghost Cells*/
		for (i = 0; i < nghb_cnt; ++i) {
			/*NON BLOCKING Communication using DERIVED DATATYPES*/
			MPI_Isend(direc1, 1, sendtype[i], nghb_to_rank[i], 0, MPI_COMM_WORLD, request + i);
			MPI_Irecv(direc1, 1, recvtype[i], nghb_to_rank[i], 0, MPI_COMM_WORLD, request + nghb_cnt + i);

			/*MPI_SENDRECV using DERIVED DATATYPES*/
//			MPI_Sendrecv(direc1, 1, sendtype[i], nghb_to_rank[i], 0, direc1, 1, recvtype[i],
//					nghb_to_rank[i], 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

			/*NON BLOCKING Communication using BUFFERS*/
//			MPI_Isend(sendbuf[i], send_cnt[i], MPI_DOUBLE, nghb_to_rank[i], 0, MPI_COMM_WORLD, request + 2 * i);
//			MPI_Irecv(recvbuf[i], recv_cnt[i], MPI_DOUBLE, nghb_to_rank[i], 0, MPI_COMM_WORLD, request + 2 * i + 1);
		}
		/*WAIT call for NON BLOCKING*/
		for (i = 0; i < 2 * nghb_cnt; ++i) {
			MPI_Wait(request + i, MPI_STATUS_IGNORE);
		}
		/*Loading Buffer values into Direc1*/
//		for (i = 0; i < nghb_cnt; ++i) {
...
//		}
		...
		if (nor1 == 1) {
			...
			/*Summing up value of OCC in all processes*/
...
//      compute the new residual
		...
		/*Summing up values of cnorm and omega in all processes*/
		...
		/*Summing up residual to check condition*/
...
//	double t2 = MPI_Wtime();
//	if(myrank==0)printf("%d Total Computation Time: %f\n", t2 - t1);
