...
int write_result(char *infilename, char *outfilename, int nintci, int nintcf,
		double *var, int iter, double ratio, float ptime, float mflops) {
	...
	fprintf(fp, "%d %e\n", iter, ratio);
	/*fprintf(fp, "     ==================================================================\n\n");
	fprintf(fp, "     L2 Cache Miss -----  L2 Cache Access ------ L2 Cache Miss Rate\n");
	fprintf(fp, "     %lld -----  %lld ------ %f\n", values[0],values[1],((double)values[0]/values[1]));
	fprintf(fp, "     ==================================================================\n\n");
	fprintf(fp, "     L3 Cache Miss -----  L3 Cache Access ------ L3 Cache Miss Rate\n");
	fprintf(fp, "     %lld -----  %lld ------ %f\n", values[2],values[3],((double)values[2]/values[3]));*/
	fprintf(fp, "     ==================================================================\n\n");
	...
	for (n = 1; n <= 10; n++)
		fprintf(fp, "%lf %lf %lf %lf\n", ipoint[n], var[n],
				ipoint[nintcf - n + 1], var[nintcf - n + 1]);
...
int write_result_vtk(char *outFileName, int startIdx, int endIdx,
	...
	for (i = startIdx; i <= endIdx; i++) {
		fprintf(fp, "8 %d %d %d %d %d %d %d %d\n", elems[0][i] - 1,
				elems[1][i] - 1, elems[3][i] - 1, elems[2][i] - 1,
				elems[4][i] - 1, elems[5][i] - 1, elems[7][i] - 1,
				elems[6][i] - 1);
	}
